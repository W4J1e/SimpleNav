...
考虑到默认使用必应日图，常常会出现必应日图的色块过于复杂，影响链接卡片的对比度，导致阅读不便，于是我重构了 UI ，默认使用渐变色背景（动态效果会导致搜索按钮等展开动画卡顿，所以换成了静态的 SVG 绘制），且把链接卡片变成了链接图标，接下来还会继续优化。

![新主页](./statics/NewUi.jpg)

![旧主页](./statics/main.jpg)
...
│   │   ├── debug/        # 调试API
│   │   ├── favicon/      # Favicon代理API
│   │   ├── movie-calendar/ # 电影日历API
...
│   └── page.tsx        # 主页面
├── components/         # React 组件
│   ├── DynamicBackground.tsx    # 动态渐变背景组件
│   ├── Footer.tsx               # 页脚组件
│   ├── Header.tsx               # 头部组件
│   ├── LinkForm.tsx             # 链接表单组件
│   ├── LinksGrid.tsx            # 链接网格组件（含小组件卡片）
│   ├── MovieCalendarDialog.tsx  # 电影日历对话框
│   ├── Search.tsx               # 搜索组件
...
因为本地开发和生产环境常常不同，所以dev 分支用于测试生产环境的部署，并非真正用于开发。
